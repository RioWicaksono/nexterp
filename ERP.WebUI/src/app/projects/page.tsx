    ...
    <AppShell>
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <button className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-medium">+ New Project</button>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard icon={Briefcase} label="Active Projects" value="0" color="blue" />
          <StatCard icon={Users} label="Team Members" value="0" color="emerald" />
          <StatCard icon={Clock} label="Pending Tasks" value="0" color="purple" />
          <StatCard icon={CheckCircle} label="Completed" value="0" color="amber" />
        </div>

        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-6">
          <h3 className="text-lg font-semibold text-slate-800 dark:text-white mb-4">Active Projects</h3>
