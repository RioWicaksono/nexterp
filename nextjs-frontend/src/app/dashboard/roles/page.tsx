  ...
  return (
    <div>
      ...
      <PageHeader title="Roles & Permissions" subtitle="Manage user roles and access control">
        <button onClick={() => { setEditingRole(null); setFormData({ name: '', description: '' }); setShowModal(true); }} className="flex items-center gap-2 px-3 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm transition">
                ...
                <tr key={role.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/30">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex flex-wrap gap-1">
                      ...
                      {role.permissions.length > 4 && <span className="px-2 py-0.5 text-xs text-slate-400">+{role.permissions.length - 4}</span>}
                    </div>
                  </td>
                  ...
                  <td className="px-4 py-3 text-right">
                    <button onClick={() => openEdit(role)} className="p-1.5 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded"><Edit2 className="w-4 h-4" /></button>
                    {!role.isSystemRole && (
          ...
          <div className="bg-white dark:bg-slate-800 rounded-xl shadow-xl w-full max-w-md mx-4" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-4 space-y-4">
              ...
              <div>
                ...
                <textarea value={formData.description} onChange={e => setFormData({ ...formData, description: e.target.value })} rows={2} className="w-full px-3 py-2 border rounded-lg dark:bg-slate-700 dark:border-slate-600 text-sm" />
              </div>
            </div>
      ...
      <ConfirmDialog isOpen={!!deleteId} title="Delete Role?" message="This action cannot be undone." confirmText="Delete" onConfirm={handleDelete} onCancel={() => setDeleteId(null)} />
    </div>
  );
}
