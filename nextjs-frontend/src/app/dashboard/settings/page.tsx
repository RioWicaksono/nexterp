          ...
          <nav className="space-y-0.5">
            {tabs.map(tab => (
              <button
                ...
                className={`w-full flex items-center gap-2 px-3 py-2 rounded-lg text-sm transition ${activeTab === tab.id ? 'bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 font-medium' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800'}`}
              >
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" value={profile.firstName} onChange={e => setProfile({ ...profile, firstName: e.target.value })} className="w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-sm" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Last Name</label>
                  <input type="text" value={profile.lastName} onChange={e => setProfile({ ...profile, lastName: e.target.value })} className="w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-sm" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Email</label>
                  <input type="email" value={profile.email} onChange={e => setProfile({ ...profile, email: e.target.value })} className="w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-sm" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Phone</label>
                  <input type="text" value={profile.phone} onChange={e => setProfile({ ...profile, phone: e.target.value })} className="w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-sm" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Language</label>
                  <select value={profile.language} onChange={e => setProfile({ ...profile, language: e.target.value })} className="w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-sm">
                ...
                <div>
                  ...
                  <select value={profile.timezone} onChange={e => setProfile({ ...profile, timezone: e.target.value })} className="w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-sm">
          ...
          {activeTab === 'notifications' && (
            <div className="space-y-4">
              ...
              {[
                ...
                { key: 'marketing' as const, label: 'Marketing', desc: 'Product updates and announcements' },
              ].map(item => (
                <div key={item.key} className="flex items-center justify-between py-3 border-b border-slate-100 dark:border-slate-700 last:border-0">
                  ...
                  <button
                    ...
                    className={`relative w-11 h-6 rounded-full transition ${notifications[item.key] ? 'bg-blue-600' : 'bg-slate-300 dark:bg-slate-600'}`}
                  >
          ...
          {activeTab === 'appearance' && (
            <div className="space-y-6">
                ...
                <div className="flex gap-3">
                  {[
                    ...
                    <button
                      ...
                      className={`flex-1 p-4 rounded-xl border-2 transition ${appearance.theme === theme.value ? 'border-blue-500' : 'border-transparent ' + theme.border}`}
                    >
              ...
              <div className="flex items-center justify-between py-3">
                ...
                <button
                  onClick={() => setAppearance(a => ({ ...a, compact: !a.compact }))}
                  className={`relative w-11 h-6 rounded-full transition ${appearance.compact ? 'bg-blue-600' : 'bg-slate-300 dark:bg-slate-600'}`}
                >
